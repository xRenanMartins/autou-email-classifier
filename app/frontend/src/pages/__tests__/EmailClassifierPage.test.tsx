...
describe('EmailClassifierPage', () => {
  ...
  it('shows form with text input and file upload', () => {
    ...
    expect(screen.getByRole('button', { name: /Classificar Email/i })).toBeInTheDocument()
  })
  ...
  it('validates text input minimum length', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('O texto deve ter pelo menos 10 caracteres')).toBeInTheDocument()
    })
  })
  ...
  it('enables submit button when valid content is provided', async () => {
    ...
    fireEvent.change(textInput, { target: { value: 'Este é um email de teste com conteúdo suficiente para validação.' } })
  ...
  it('shows loading state during processing', async () => {
    ...
    fireEvent.change(textInput, { target: { value: 'Email de teste com conteúdo suficiente.' } })
  ...
  it('validates form inputs correctly', async () => {
    ...
    fireEvent.change(textInput, { target: { value: 'Este é um texto válido com mais de 10 caracteres.' } })
    
    await waitFor(() => {
      expect(screen.queryByText('O texto deve ter pelo menos 10 caracteres')).not.toBeInTheDocument()
    })
  })
})
