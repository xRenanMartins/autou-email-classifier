...
class InMemoryEmailRepository(EmailRepositoryPort):
    ...
    async def get_by_classification(
        ...
        for email in self._emails.values():
            if email._classification and email._classification.label == label:
    ...
    async def get_processing_stats(self) -> Dict[str, Any]:
        ...
        for email in self._emails.values():
            if email._classification:
                ...
                if (
                    hasattr(email._classification, "processing_time_ms")
                    and email._classification.processing_time_ms is not None
                ):
    ...
    async def search(
        ...
        for email in self._emails.values():
            # Filtra por label
            if label and (
                not email._classification or email._classification.label != label
            ):
                continue
...
class InMemoryTemplateRepository(TemplateRepositoryPort):
    ...
    async def get_active_templates(self) -> List[ResponseTemplate]:
        ...
        return [template for template in self._templates.values() if template.is_active]
