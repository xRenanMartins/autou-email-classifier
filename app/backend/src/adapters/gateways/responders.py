...
from ...core.domain.entities import (
    Email, EmailLabel, Classification, 
...
class TemplateResponder(ResponderPort):
    ...
    async def customize_response(
        self,
        base_response: SuggestedResponse,
...
class OpenAIResponder(ResponderPort):
    ...
    async def suggest_reply(
        ...
        try:
            ...
            # Parse da resposta
            suggested_response = self._parse_openai_response(response, email, classification)
            ...
            return await template_responder.suggest_reply(email, classification, context)
    ...
    async def customize_response(
        ...
        try:
            ...
            template_responder = TemplateResponder()
            return await template_responder.customize_response(base_response, customizations)
    ...
    async def _call_openai_api(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "Você é um assistente especializado em respostas de email."},
...
class HuggingFaceResponder(ResponderPort):
    ...
    async def suggest_reply(
        ...
        try:
            ...
            # Parse da resposta
            suggested_response = self._parse_hf_response(response, email, classification)
    ...
    def _create_generation_prompt(
        ...
        """Cria prompt para geração de resposta."""
        if classification.label == EmailLabel.PRODUCTIVE:
            prompt = f"Email sobre: {email.subject or 'solicitação'}. Resposta profissional:"
        else:
            prompt = f"Email de agradecimento. Resposta cordial:"
