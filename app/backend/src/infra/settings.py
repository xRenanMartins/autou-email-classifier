...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        "sqlite:///./emails.db", description="URL de conexão com banco de dados"
    )
    ...
    huggingface_token: Optional[str] = Field(None, description="Token do Hugging Face")

    # Configurações de cache
    redis_url: Optional[str] = Field(None, description="URL de conexão com Redis")
    ...
    rate_limit_enabled: bool = Field(
        True, description="Se rate limiting está habilitado"
    )
    ...
    health_check_interval: int = Field(
        30, description="Intervalo de health check em segundos"
    )

    # Configurações de segurança adicional
    enable_trusted_hosts: bool = Field(
        True, description="Se TrustedHostMiddleware está habilitado"
    )
    ...
    def get_security_config(self) -> dict:
        ...
        return {
            ...
            "cors": {"enabled": self.enable_cors, "origins": self.get_cors_origins()},
        }
    ...
    def get_monitoring_config(self) -> dict:
        ...
        return {
            ...
            "health_check": {"interval": self.health_check_interval},
