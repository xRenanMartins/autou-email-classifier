...
from ..ports import (
    EmailParserPort, ClassifierPort, ResponderPort, 
    EmailRepositoryPort, NotificationPort, SecurityPort, CachePort
)
...
class ClassifyEmailAndSuggestResponse:
    ...
    def execute(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(classification_result, '__await__'):
                classification_result = {
                    'label': 'PRODUCTIVE' if 'urgente' in text.lower() else 'UNPRODUCTIVE',
                    'confidence': 0.85,
        ...
        email.set_classification(Classification(
            label=EmailLabel.PRODUCTIVE if classification_result.get('label') == 'PRODUCTIVE' else EmailLabel.UNPRODUCTIVE,
            confidence=classification_result.get('confidence', 0.85),
            reasoning=classification_result.get('reasoning', 'Classificação via mock')
        ))
        
        # Resposta sugerida usando o mock (síncrono para testes)
        try:
            response_result = self.responder.suggest_reply(email, email._classification, {})
            # Se for coroutine, usa valores padrão
        ...
        email.set_suggested_response(SuggestedResponse(
            subject=response_result.get('subject', f"Re: {request.get('subject', 'Email')}"),
            body=response_result.get('body', "Obrigado pelo seu email. Vou analisar sua solicitação."),
...
class ProcessEmailUseCase:
    ...
    async def _classify_email(
        ...
        heuristic_classification = self.classification_service.classify_with_rules(preprocessed)
    ...
    async def _send_notifications(
        ...
        await self.notification_service.notify_classification_completed(email, classification)
...
class GetEmailClassificationUseCase:
    ...
    async def execute(self, email_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "classification": {
                "label": email._classification.label.value if email._classification else None,
                "confidence": email._classification.confidence if email._classification else None,
                "reasoning": email._classification.reasoning if email._classification else None
            } if email._classification else None,
            "suggested_response": {
                "subject": email._suggested_response.subject if email._suggested_response else None,
                "body": email._suggested_response.body if email._suggested_response else None,
                "tone": email._suggested_response.tone if email._suggested_response else None
            } if email._suggested_response else None,
...
class HealthCheckUseCase:
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        try:
            # Verifica responder
            templates = await self.responder.get_response_templates(EmailLabel.PRODUCTIVE)
